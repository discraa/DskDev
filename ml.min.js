...
var parse = function(json) {
    // DSK MODIFICATION
    // Cool packet events (post at end)
    dsk.emit(`prePacket`, json);
    dsk.emit(`prePacket:${json.type}`, json);
    ...
    dsk.emit(`postPacket`, json);
    dsk.emit(`postPacket:${json.type}`, json);
  },
  ...
  setup_mobile_input = (jv.net_timer_function = function() {
    ...
    if (!editing) {
      ...
      if (-1 == me) send({
        ...
        // DSK MODIFICATION
        // Custom commands
        const prefixes = Array.from(dsk.commands.keys());
        const prefix = prefixes.find(e => e === t[0]);
        
        // If command found
        if (prefix) {
            const callback = dsk.commands.get(prefix);
            
            // Strip away the prefix bc why not
            const context = e.slice(e.indexOf(' ') + 1);
            
            // If no context them fuck it
            const result = callback(e.includes(' ') ? context : '');
            return;
        }
